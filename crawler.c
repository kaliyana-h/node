...
char *start_page = "https://news.google.com";
 ...
 while(still_running && !pending_interrupt) {
   ...
   while((m = curl_multi_info_read(multi_handle, &msgs_left))) {
     if(m->msg == CURLMSG_DONE) {
       ...
       char *path = "localhost";
       //char *api = "/api/v2/job";
       //char *message_fmt = "POST /apikey=api/v2/job&command=%s HTTP/1.0\r\n\r\n";
       ...
       //char *lUrl;
       //sprintf(lUrl, "/api/v2/job?source=%s", url);
       int lenUrl = strlen(url);
       sprintf(sendUrl, "POST /api/v2/job?source=%s HTTP/1.0\r\n\r\n", url);       
       fprintf(stdout, sendUrl);
       fflush(stdout);
       int total = strlen(sendUrl);
       int sent = 0;
       do {
           int bytes = send(sockfd,sendUrl+sent,total-sent,0);
           if (bytes < 0)
